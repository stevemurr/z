...
_z_env_copy() {
    ...
    # Find the variable in remote data
    local found=false
    local value=""
    local secret=false
    local description=""
    ...
    echo "${remote_data}" | while IFS= read -r line; do
        if [[ "${line}" =~ '"name": "'"${name}"'"' ]]; then
            # Extract value, secret, description from this line
            if [[ "${line}" =~ '"value": "([^"]*)"' ]]; then
                value="${match[1]}"
            fi
            if [[ "${line}" =~ '"secret": true' ]]; then
                secret=true
            fi
            ...
            _z_env_add "${args[@]}"
            echo "Copied ${name} from ${machine}"
            found=true
            return 0
        fi
    done

    # Check if we found it (subshell issue workaround)
    if ! echo "${remote_data}" | grep -q "\"name\": \"${name}\""; then
        echo "Error: Variable '${name}' not found on ${machine}"
        return 1
    fi

    # Parse and add (since while loop is in subshell)
    local line=$(echo "${remote_data}" | grep -A0 "\"name\": \"${name}\"")
